...
class MSMT17(BaseImageDataset):
    ...
    def __init__(self, root='', format_tag='tensor', pretrained='CLIPreidFinetune', pid_begin=0, **kwargs):
        ...
        self.list_train_path = osp.join(root, self.root_folder, 'list_train.txt')
        self.list_val_path = osp.join(root, self.root_folder, 'list_val.txt')
        self.list_query_path = osp.join(root, self.root_folder, 'list_query.txt')
        self.list_gallery_path = osp.join(root, self.root_folder, 'list_gallery.txt')
    ...
    def _process_dir(self, dir_path, list_path):
        ...
        for img_idx, img_info in enumerate(lines):
            ...
            pre, ext = osp.splitext(img_path)
            if self.tag == 'tensor' and ext != '.pt':
                img_path = pre + '.pt'
            img_path = osp.join(dir_path, img_path)
            dataset.append((img_path, self.pid_begin+pid, camid-1, 'rgb'))
